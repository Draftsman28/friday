...
SEARCH_SCRIPT = Path.home() / "friday" / "core" / "search_friday.py"
...
def find_lm_studio():
    ...
    CONFIG_FILE.parent.mkdir(parents=True, exist_ok=True)
    CONFIG_FILE.write_text(manual)
    return manual

def open_image_single(filename, idx, total):
    path = str(IMAGE_DIR / filename)
    title = f"{WINDOW_TITLE_BASE}-{idx}"
    win_w = 3328 // max(total, 1)
    win_x = XP_X + (idx * win_w)

    if shutil.which("feh"):
        cmd = ["feh", "--title", title, "--geometry", f"{win_w}x800+{win_x}+{XP_Y}", "--scale-down", path]
        try:
            ...
            if shutil.which("xdotool"):
                time.sleep(0.3)
                subprocess.Popen(
                    f"winid=$(xdotool search --name '^{title}$' | head -1); [ -n \"$winid\" ] && xdotool windowmove $winid {win_x} {XP_Y}",
...
def search(query):
    result = subprocess.run([sys.executable, str(SEARCH_SCRIPT), query], capture_output=True, text=True, timeout=30)
